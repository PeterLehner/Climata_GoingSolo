...
df_coefficients = pd.read_csv('Data/coefficients_sqft_to_kwh.csv')
...
def calculate_savings(dict_working, zip_query, electric_bill_query, loan_term_query, loan_rate_query, heatpump_query, sqft_query): 
    ...
    elif electric_bill_query is None and sqft_query is not None:
        slope = df_coefficients[df_coefficients['state'] == state]['slope'].squeeze()
        intercept = df_coefficients[df_coefficients['state'] == state]['intercept'].squeeze()
        electricity_use_kwh = slope * sqft_query + intercept
    else:
